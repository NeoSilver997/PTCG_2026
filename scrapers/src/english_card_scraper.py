#!/usr/bin/env python3
"""
English (Asia/SG) Pokemon Card Scraper for PTCG_2026
Scrapes card details from asia.pokemon-card.com/sg and saves to data/cards JSON files

Features:
- Multi-threaded scraping with rate limiting
- HTML caching for offline processing
- Automatic expansion grouping
- Maps English data to PTCG_2026 schema

Sample usage:
    python scrapers/src/english_card_scraper.py --id-range 1000 100 --cache-html
"""
...
class EnglishCardScraper:
    """Scraper for English (Asia) Pokemon card details"""
    ...
    def __init__(self, data_root: Optional[str] = None):
        ...
        self.cards_dir = self.data_root / 'cards' / 'english'
    ...
    def _extract_card_info(self, soup: BeautifulSoup, url: str) -> Optional[Dict[str, Any]]:
        ...
        if evolution_stage_text:
            card_name = re.sub(r'^(Basic|Stage\s*1|Stage\s*2)\s*', '', card_name, flags=re.IGNORECASE).strip()
    ...
    def _extract_rarity(self, soup: BeautifulSoup) -> Optional[str]:
        # Similar logic to HK/JP but adapted for English text if needed
    ...
    def _extract_supertype(self, soup: BeautifulSoup, card_name: str, page_text: str) -> str:
        ...
        if 'Energy' in card_name: return 'ENERGY'
        section_titles = ' '.join(tag.get_text(strip=True) for tag in soup.select('h2,h3'))
        if any(keyword in section_titles for keyword in ['Attack', 'Ability']): return 'POKEMON'
        return 'TRAINER'

    def _extract_hp(self, soup: BeautifulSoup) -> Optional[int]:
        match = re.search(r'HP\s*(\d+)', soup.get_text())
    ...
    def _extract_evolution_stage_from_marker(self, marker_text: Optional[str]) -> Optional[str]:
        ...
        # English patterns
        if marker_text == 'Basic':
            return 'BASIC'
        if marker_text == 'Stage 1':
            return 'STAGE_1'
        if marker_text == 'Stage 2':
            return 'STAGE_2'
    ...
    def _extract_evolution_stage(self, soup: BeautifulSoup, text: str) -> Optional[str]:
        ...
        if not type_span: return 'BASIC'
        stage_text = type_span.get_text(strip=True)
        if 'Basic' in stage_text: return 'BASIC'
        if 'Stage 1' in stage_text: return 'STAGE_1'
        if 'Stage 2' in stage_text: return 'STAGE_2'
        if 'VMAX' in stage_text: return 'VMAX'
        if 'VSTAR' in stage_text: return 'VSTAR'
        return 'BASIC'
    ...
    def _determine_subtype(self, supertype: str, evolution_stage: Optional[str], soup: BeautifulSoup, page_text: str) -> Optional[str]:
        ...
        if supertype == 'ENERGY': return 'BASIC_ENERGY' if 'Basic' in page_text else 'SPECIAL_ENERGY'
        
        section_titles = ' '.join(tag.get_text(strip=True) for tag in soup.select('h2,h3') if tag)
        if 'Tool' in section_titles: return 'TOOL'
        if 'Stadium' in section_titles: return 'STADIUM'
        if 'Supporter' in section_titles: return 'SUPPORTER'
        return 'ITEM'
    ...
    def _format_web_card_id(self, card_id: Optional[str]) -> Optional[str]:
        return f"en{int(card_id)}" if card_id and card_id.isdigit() else None
...
def build_card_url(card_id: int) -> str:
    # Using hk-en region instead of sg
    return f"https://asia.pokemon-card.com/hk-en/card-search/detail/{card_id}/"
...
def main():
    parser = argparse.ArgumentParser(description='English (Asia) Card Scraper')
    parser.add_argument('--ids', type=str, help='Comma-separated card IDs')
    parser.add_argument('--id-range', nargs=2, type=int)
    parser.add_argument('--output', default='english_cards.json')
