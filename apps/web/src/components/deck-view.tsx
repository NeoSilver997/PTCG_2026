...
// Colour map keyed on the exact effectTag strings written to the DB
// (generated by populate-effect-tags.ts + manual backfill scripts)
const EFFECT_TAG_COLORS: Record<string, string> = {
  // ── Draw / Search ──────────────────────────────────────
  '少量抽卡':       '#a5f3fc',  // cyan-200  — draw 1
  '抽卡效果':       '#06b6d4',  // cyan-500  — draw 2-3
  '大量抽卡':       '#0ea5e9',  // sky-500   — draw 3+
  '牌庫搜索':       '#10b981',  // emerald-500
  '棄牌搜索':       '#059669',  // emerald-600
  '搜索效果':       '#10b981',  // emerald-500 (legacy alias)
  '情報收集':       '#14b8a6',  // teal-500
  '牌庫操作':       '#15803d',  // green-700
  '牌庫重洗':       '#16a34a',  // green-600
  // ── Energy ─────────────────────────────────────────────
  '能量操作':       '#fb923c',  // orange-400
  '能量附著':       '#f97316',  // orange-500
  '能量回收':       '#f59e0b',  // amber-500
  '能量條件':       '#fbbf24',  // amber-400
  '附上搜索能量':   '#f97316',  // orange-500 — attach/search energy
  '搜索指定能量':   '#ea580c',  // orange-600 — typed energy search
  // ── Damage ─────────────────────────────────────────────
  '傷害效果':       '#ef4444',  // red-500
  '條件傷害':       '#dc2626',  // red-600
  '連鎖傷害':       '#b91c1c',  // red-700
  '傷害指示物':     '#f87171',  // red-400
  '備戰傷害加成':   '#fca5a5',  // red-300
  '棄牌區傷害加成': '#e11d48',  // rose-600
  '反噬傷害':       '#9f1239',  // rose-800
  '昏厥條件':       '#d946ef',  // fuchsia-500
  '最大傷害':       '#f43f5e',  // rose-500
  // ── Defense ────────────────────────────────────────────
  '傷害防禦':       '#6366f1',  // indigo-500
  '傷害減免':       '#818cf8',  // indigo-400
  '高額傷害減免':   '#4f46e5',  // indigo-600
  '效果免疫':       '#3730a3',  // indigo-800
  '全體防禦':       '#4338ca',  // indigo-700
  '特定寶可夢防禦': '#a5b4fc',  // indigo-300
  '屬性防禦':       '#c7d2fe',  // indigo-200
  '無視弱點/效果':  '#0d9488',  // teal-600
  '弱點消除':       '#0f766e',  // teal-700
  '弱點改變':       '#115e59',  // teal-800
  'HP提升':         '#5eead4',  // teal-300
  // ── Status ─────────────────────────────────────────────
  '狀態異常':       '#ec4899',  // pink-500
  '狀態恢復':       '#f9a8d4',  // pink-300
  '狀態免疫':       '#be185d',  // pink-700
  '狀態施加':       '#db2777',  // pink-600
  // ── Control ────────────────────────────────────────────
  '道具消除':       '#84cc16',  // lime-500
  '道具移除':       '#65a30d',  // lime-600
  '招式封鎖':       '#ca8a04',  // yellow-600
  '招式鎖定':       '#b45309',  // amber-700
  '招式複製':       '#a3e635',  // lime-400
  '招式複製對手':   '#4d7c0f',  // lime-800
  '撤退封鎖':       '#eab308',  // yellow-500
  '撤退干擾':       '#facc15',  // yellow-400
  '附著干擾':       '#d97706',  // amber-600
  '物品卡封鎖':     '#92400e',  // amber-800
  '支援者限制':     '#78350f',  // amber-900
  '能量需求增加':   '#b45309',  // amber-700
  // ── Prize / Positioning ────────────────────────────────
  '獎賞控制':       '#22c55e',  // green-500
  '切換效果':       '#7c3aed',  // violet-700
  '連續技':         '#9333ea',  // purple-600
  '硬幣判定':       '#a855f7',  // purple-500
  // ── Evolution / Setup ──────────────────────────────────
  '放置基礎寶可夢': '#4ade80',  // green-400 — bench Basic from deck
  '進化支援':       '#8b5cf6',  // violet-500
  // ── Special / Misc ─────────────────────────────────────
  '放置效果':       '#3b82f6',  // blue-500
  '丟棄效果':       '#64748b',  // slate-500
  '手牌丟棄':       '#475569',  // slate-600
  '手牌回收':       '#94a3b8',  // slate-400
  '特性效果':       '#0284c7',  // sky-600
  '進化效果':       '#7e22ce',  // purple-800
  '撤退效果':       '#94a3b8',  // slate-400
  '競技場效果':     '#0369a1',  // sky-700
  '場地增幅':       '#075985',  // sky-800
  '回復效果':       '#2dd4bf',  // teal-400
  '其他效果':       '#334155',  // slate-700
};

const TAG_DEFAULT_COLOR = '#64748b'; // slate

// ── Tag grouping for distribution display ────────────────────────
const TAG_GROUPS: Array<{ label: string; icon: string; tags: string[]; mustHave?: boolean }> = [
  { label: '抽牌 & 搜索', icon: '🃏', mustHave: true,
    tags: ['少量抽卡', '抽卡效果', '大量抽卡', '牌庫搜索', '棄牌搜索', '情報收集', '牌庫操作', '牌庫重洗', '搜索效果'] },
  { label: '出場準備', icon: '🐾', mustHave: true,
    tags: ['放置基礎寶可夢', '放置效果', '進化支援', '進化效果'] },
  { label: '傷害引擎', icon: '⚔️', mustHave: true,
    tags: ['傷害效果', '條件傷害', '傷害指示物', '連鎖傷害', '備戰傷害加成', '棄牌區傷害加成', '反噬傷害', '昏厥條件', '最大傷害'] },
  { label: '能量', icon: '⚡',
    tags: ['能量操作', '能量附著', '能量回收', '能量條件', '附上搜索能量', '搜索指定能量'] },
  { label: '防禦 & 弱點', icon: '🛡',
    tags: ['傷害防禦', '傷害減免', '高額傷害減免', '效果免疫', '全體防禦', '特定寶可夢防禦', '屬性防禦', 'HP提升', '無視弱點/效果', '弱點消除', '弱點改變'] },
  { label: '干擾 & 控制', icon: '🔒',
    tags: ['道具消除', '道具移除', '招式封鎖', '招式鎖定', '招式複製', '招式複製對手', '撤退封鎖', '撤退干擾', '附著干擾', '物品卡封鎖', '支援者限制', '能量需求增加', '獎賞控制'] },
  { label: '狀態', icon: '🌀',
    tags: ['狀態異常', '狀態施加', '狀態恢復', '狀態免疫'] },
  { label: '特殊 & 其他', icon: '✨',
    tags: ['特性效果', '切換效果', '連續技', '硬幣判定', '手牌丟棄', '手牌回收', '丟棄效果', '撤退效果', '競技場效果', '場地增幅', '回復效果', '其他效果'] },
];
...
export function EffectTagSummary({ entries }: { entries: DeckCardEntry[] }) {
  ...
  for (const entry of entries) {
    ...
    for (const tag of tags) {
      ...
      if (displayName && !rec.cards.includes(displayName)) rec.cards.push(displayName);
    }
  }
  ...
  const maxQty = Math.max(...Array.from(tagCounts.values()).map(v => v.qty));
  // Tags not in any group (new/unknown)
  const groupedTags = new Set(TAG_GROUPS.flatMap(g => g.tags));
  const ungrouped = Array.from(tagCounts.keys()).filter(t => !groupedTags.has(t));

  const allGroups = [
    ...TAG_GROUPS,
    ...(ungrouped.length ? [{ label: '未分類', icon: '❓', tags: ungrouped }] : []),
  ];

  // Active groups = those with at least one tag present in this deck
  const activeGroups = allGroups
    .map(g => ({
      ...g,
      activeTags: g.tags.filter(t => tagCounts.has(t)),
    }))
    .filter(g => g.activeTags.length > 0);

  // Flat sorted list for compact view (top 12 by qty)
  const flatSorted = Array.from(tagCounts.entries())
    .sort((a, b) => b[1].qty - a[1].qty)
    .slice(0, 12);

  return (
    <div>
      {/* Compact flat view (always visible) */}
      <div className="flex flex-wrap gap-1.5 mb-2">
        {flatSorted.map(([tag, { qty, cards }]) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip */}
              <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 pointer-events-none min-w-[140px]">
                <div className="bg-slate-900 border border-slate-600 rounded-lg shadow-xl px-2.5 py-2 text-[11px] text-slate-200">
                  <div className="flex items-center justify-between mb-1.5">
                    <span style={{ color }} className="font-semibold">{tag}</span>
        ...
        {tagCounts.size > 12 && (
          <span className="text-slate-500 text-xs self-center">+{tagCounts.size - 12} 更多</span>
        )}
      </div>
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-slate-400 hover:text-slate-200 text-xs transition-colors mt-1"
      >
        <span>{expanded ? '▲' : '▼'}</span>
        <span>{expanded ? '收起分布圖' : '查看效果分布'}</span>
      </button>

      {/* Expanded grouped distribution */}
      {expanded && (
        <div className="mt-3 space-y-3">
          {activeGroups.map(group => {
            const groupTotal = group.activeTags.reduce((s, t) => s + (tagCounts.get(t)?.qty ?? 0), 0);
            return (
              <div key={group.label}>
                {/* Group header */}
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-base leading-none">{group.icon}</span>
                  <span className="text-slate-300 text-xs font-semibold">{group.label}</span>
                  <span className="text-slate-500 text-xs">({groupTotal} 槽)</span>
                  {group.mustHave && (
                    <span className="text-amber-400 text-[9px] border border-amber-500/40 bg-amber-500/10 px-1 rounded leading-none py-0.5">核心</span>
                  )}
                </div>
                {/* Tag rows */}
                <div className="space-y-1 pl-1">
                  {group.activeTags
                    .sort((a, b) => (tagCounts.get(b)?.qty ?? 0) - (tagCounts.get(a)?.qty ?? 0))
                    .map(tag => {
                      const { qty, cards } = tagCounts.get(tag)!;
                      const color = EFFECT_TAG_COLORS[tag] ?? TAG_DEFAULT_COLOR;
                      ...
                      return (
                        <div key={tag} className="group relative flex items-center gap-2">
                          ...
                          {/* Count + pct */}
                          <div className="flex-none flex items-center gap-1.5 text-[11px]">
                            <span style={{ color }} className="font-semibold">×{qty}</span>
